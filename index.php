...
<?php
/* if (count($_POST) > 0){
    if (!empty($_POST['nom']) && empty($_POST['prenom']) && empty($_POST['date de naissance']) && empty($_POST['pays de naissance'])
    && empty($_POST['nationalité']) && empty($_POST['adresse']) && empty($_POST['email']) &&
     empty($_POST['telephone']) && empty($_POST['diplome']) && empty($_POST['numero']) && empty($_POST['nombre']) && empty($_POST['text1'])
     && empty($_POST['text2']) && empty($_POST['text3']) && empty($_POST['button'])){

       if(preg_match($nom, $_POST['nom']) && (preg_match($prenom, $_POST['prenom']) && (preg_match($dateDeNaissance , $_POST['date de naissance'])
        && (preg_match($paysDeNaissance, $_POST['pays de naissance']) && (preg_match($nationalité, $_POST['nationalite'])
        && (preg_match($adresse, $_POST['adresse']) && (preg_match($email, $_POST['email']) && (preg_match($telephone, $_POST['telephone'])
        && (preg_match($diplome, $_POST['diplome']) && (preg_match($numero, $_POST['numero']) && (preg_match($nombre, $_POST['nombre'])
        && (preg_match($text1, $_POST['text1']) && (preg_match($text2, $_POST['text2'])
        && (preg_match($text3, $_POST['text3'])){
?>
          <p>Nom: <?= $_POST['nom'] ?></p>
          <p>Prenom: <?= $_POST['prenom'] ?></p>
          <p>Date de naissance: <?= $_POST['date de naissance'] ?></p>
          <p>Pays de naissance: <?= $_POST['pays de naissance'] ?></p>
          <p>Nationalité: <?= $_POST['nationalité'] ?></p>
          <p>Adresse: <?= $_POST['adresse'] ?></p>
          <p>Email: <?= $_POST['email'] ?></p>
          <p>Téléphone: <?= $_POST['telephone'] ?></p>
          <p>Diplôme: <?= $_POST['diplome'] ?></p>
          <p>Numéro: <?= $_POST['numero'] ?></p>
          <p>Nombre: <?= $_POST['nombre'] ?></p>
          <p>Text 1: <?= $_POST['text1'] ?></p>
          <p>Text 2: <?= $_POST['text2'] ?></p>
          <p>Text 3: <?= $_POST['text3'] ?></p>
<?php
        }
}
  }else{
  */?>
  ...
  <div class="container">
    <div class="col-sm-8 mt-4">
      <form class="nom" action="cible.php" method="post">
        ...
        <div class="form-group">
          ...
          <input type="text" name="date_de_naissance" placeholder="date de naisance">
        </div>
        <div class="form-group">
          <label class="col-sm-3 col-form-label" name="pays de naissance">Pays de naissance: </label>
          <select name="pays">
        ...
        <div class="form-groupe">
          ...
          <select name="nationalite">
            <option value="Francaise" selected="selected">Française </option>
        ...
        <div class="form-group">
          <label class="col-sm-3 col-form-label">Dipôme: </label>
          <select name="diplome">
            <option>Sans</option>
            <option>Bac</option>
            <option>Bac+2</option>
        ...
        <div class="form-group">
          <label class="col-sm-3 col-form-label">Numéro de pôle emploi: </label>
          <input type="text" name="numero" placeholder="numero pôle emploi">
        </div>
        <div class="form-group">
          <label class="col-sm-3 col-form-label">Nombre de badge: </label>
          <input type="text" name="nombre" placeholder="nombre de badge">
        </div>
        <div class="form-group">
          <a href="https://www.codecademy.com/fr">Lien codecademy</a>
          </div>
        <div class="form-group">
          <label>Si vous étiez un super héros/une super héroïne, qui seriez-vous et pourquoi?</label>
          <textarea name="text1" rows="6" cols="60"></textarea>
        </div>
        <div class="form-group">
          <label>Racontez-nous un de vos "hacks" (pas forcément technique ou informatique)</label>
          <textarea name="text2" rows="6" cols="60"></textarea>
        </div>
        <div class="form-group">
          <label>Avez-vous une expérience avec la programmation avant de remplir ce formulaire</label>
          <textarea name="text3" rows="6" cols="60"></textarea>
        </div>
          <div class="form-check">
            <input type="checkbox" class="form-check-input" id="exampleCheck1">
            <label class="form-check-label" for="exampleCheck1">Cocher la case</label>
          </div>
